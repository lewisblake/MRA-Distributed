...
[ knots, ~, nRegions, outputData, predictionLocations, indexMatrix, nWorkersUsed ] = build_structure_in_parallel( NUM_LEVELS_M, ...
    NUM_PARTITIONS_J, NUM_KNOTS_r, domainBoundaries, offsetPercentage, NUM_WORKERS, nLevelsInSerial, data(:,1:3), predictionVector );

% Potentially have an if-statement here that calls a function and reports
% summary statistics about the grid structure? Or include in calculation
% Type
...
switch calculationType
    ...
    case 'build_structure'
        %% Report structure summary statistics
        %
        %
        %
    otherwise
